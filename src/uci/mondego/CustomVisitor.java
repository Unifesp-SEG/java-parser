...
public class CustomVisitor extends TreeVisitor {
  ...
  public void generateInputForTest(MetricCollector collector) {
    ...
    metricString
      ...
      .append(collector.HVOC).append(internalSeparator)
      .append(collector.HEFF).append(internalSeparator)
      .append(collector.CREF).append(internalSeparator)
      .append(collector.XMET).append(internalSeparator)
      .append(collector.LMET).append(internalSeparator)
      .append(collector.NOA).append(internalSeparator)
      .append(collector.HDIF).append(internalSeparator)
      ...
      .append(collector.MDN).append(internalSeparator)
      .append(collector.NEXP).append(internalSeparator)
      .append(collector.LOOP).append(internalSeparator)
      .append(collector.NBLTRL).append(internalSeparator)
      .append(collector.NCLTRL).append(internalSeparator)
      .append(collector.NNLTRL).append(internalSeparator)
    ...
    metadataString.append(file.substring(0, file.length() - 4) + collector._methodName).append(internalSeparator)
        .append(collector._path.toString());

    String info = metadataString + internalSeparator + metricString + System.lineSeparator();
    try {
      FileWriters.metricsFileWriter.append(info);
    } catch (IOException e) {
      e.printStackTrace();
    }
  }
  ...
  public void generateInputForOreo(MetricCollector collector) {
    ...
    metadataString.append(collector._path.getParent().getFileName()).append(internalSeparator)
        ...
        .append(internalSeparator).append(collector.END_LINE).append(internalSeparator).append(collector._methodName)
        .append(internalSeparator).append(collector.NTOKENS).append(internalSeparator)
        .append(collector.tokensMap.size()).append(internalSeparator).append(collector.metricHash)
        .append(internalSeparator).append(collector.fileId).append(internalSeparator).append(collector.methodId);
    ...
    line.append(metadataString).append(externalSeparator).append(metricString).append(externalSeparator)
        ...
        .append(externalSeparator).append(methodNameActionString).append(externalSeparator).append(collector.SLOC)
  ...
  public void generateInputForScc(MetricCollector collector) {
    ...
    try {
      FileWriters.bookkeepingWriter
          .append(collector.fileId + ":" + collector._path.toString() + ";" + collector.methodId + ":"
              + collector._methodName + ";" + collector.START_LINE + ":" + collector.END_LINE + System.lineSeparator());
  ...
  public int getMethodId(Path fp, String name) {
    return 0;
  }
